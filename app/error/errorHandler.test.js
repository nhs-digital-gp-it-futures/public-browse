...
describe('errorHandler', () => {
  ...
  describe('when error comes back from api', () => {
    ...
    it('should change the message to the response error and status text when status is not 404', () => {
      expect(errorHandler(apiError)).toEqual(expect.objectContaining({
        message: 'some error server error',
      }));
    });

    it('should change the message when status is 404', () => {
      apiError.response.status = 404;
      expect(errorHandler(apiError)).toEqual(expect.objectContaining({
        message: 'Solution with id: 55 not found',
      }));
    });
