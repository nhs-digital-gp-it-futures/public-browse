...
test('should render the homepage hero', async (t) => {
  ...
  await t
    ...
    .eql('The GP IT Futures procurement framework has replaced GP System of Choice (GPSoC). Use our Buying Catalogue to find out what high-quality IT systems and services are available.');
});
...
test('should render the about us section', async (t) => {
  ...
  await t
    ...
    .expect(aboutUsSection.find('p').nth(1).innerText)
    .eql('The GP IT Futures procurement framework, which replaces the GP System of Choice (GPSoC) framework, will make a wider range of systems available. The systems have been assessed against a set of Capabilities and Standards to help improve safety, quality and efficiency in GP surgeries.')
    .expect(aboutUsSection.find('p').nth(2).innerText)
    .eql('The functionality of the Buying Catalogue will be enhanced and the number of available products – known as Solutions - will increase over time.');
});
...
test('should render the guidance promo', async (t) => {
  ...
  await t
    ...
    .expect(guidancePromo.find('p').innerText)
    .eql('Our guide explains how to use the Buying Catalogue to find and procure clinical Solutions.');
});
...
test('should render the browse promo', async (t) => {
  ...
  await t
    ...
    .expect(browsePromo.find('p').innerText)
    .eql('See what’s available on the GP IT Futures procurement framework that can meet your needs.');
});
